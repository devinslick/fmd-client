...
Usage:
    python fmd_export.py --url <server_url> --id <fmd_id> --password <password> --output <output_path> [--locations [N]] [--pictures]
...
def get_all_locations(base_url, access_token, num_to_get=-1):
...
def main():
    parser = argparse.ArgumentParser(description="FMD Server Data Export Script")
    ...
    if num_locations_to_get is None and not include_pictures:
    ...
    if num_locations_to_get is not None:
        ...
        locations_json = get_all_locations(base_url, access_token, num_locations_to_get)
    if include_pictures:
