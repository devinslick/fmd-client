...
Usage:
    python fmd_export.py --url <server_url> --id <fmd_id> --password <password> --output <output_path> [--locations] [--pictures]
...
def save_locations_csv(locations_json, out_path):
    ...
    for locationJSON in locations_json:
        loc = None
        if isinstance(locationJSON, str):
            try:
                loc = json.loads(locationJSON)
            except Exception:
                continue
        elif isinstance(locationJSON, dict):
            loc = locationJSON
        if not loc:
            continue
        date = loc.get('time', '')
    ...
    for idx, pic_b64 in enumerate(pictures_json):
        try:
            with open(os.path.join(out_dir, f"{idx}.png"), 'wb') as f:
                f.write(base64.b64decode(pad_base64(pic_b64.split(',')[-1])))
        except Exception as e:
            print(f"Warning: failed to write picture {idx}: {e}")
    ...
    if is_zip:
        ...
        with zipfile.ZipFile(output_path, 'w', zipfile.ZIP_DEFLATED) as zf:
            ...
            if include_pictures and pictures_json is not None:
                for idx, pic_b64 in enumerate(pictures_json):
                    try:
                        zf.writestr(f'pictures/{idx}.png', base64.b64decode(pad_base64(pic_b64.split(",")[-1])))
                    except Exception as e:
                        print(f"Warning: failed to write picture {idx}: {e}")
